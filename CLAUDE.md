...
1. **认证安全**: JWT Token 存储在 HttpOnly Cookie 中，前端不处理 token，防止 XSS 攻击
2. **模块化架构**: 后端每个功能领域独立成 NestJS 模块，前端采用组合式 API 和组件化设计
3. **自动请求去重**: Axios 拦截器自动处理重复请求取消，避免不必要的网络请求
4. **响应拦截器优化**: 响应拦截器直接返回 `response.data`，调用 API 时无需额外解包
...
# 开发模式 (端口 3000)
npm run start:dev  # NestJS 内置热重载
...
# 测试
npm run test                    # 运行所有测试
npm run test -- test/file.spec.ts  # 运行单个测试文件
npm run test:e2e               # 运行端到端测试

# 数据库迁移 (TypeORM)
npm run typeorm:generate src/migrations/MigrationName  # 生成迁移文件
npm run typeorm:run                                 # 执行迁移
npm run typeorm:revert                              # 回滚迁移
npm run typeorm:show                                # 查看迁移状态
...
### 环境变量
- JWT_SECRET: JWT 密钥
- 数据库连接配置在 `server/src/config/database.config.ts`
- 服务端口: 后端 3000，前端 1234
...
- **连接配置**: `server/src/config/database.config.ts`
- **数据库**: MySQL 8.0+
...
2. **前端部署**:
   ...
   - 构建项目: `npm run build`
   - 部署 `dist/` 目录到静态文件服务器（nginx、Apache等）
